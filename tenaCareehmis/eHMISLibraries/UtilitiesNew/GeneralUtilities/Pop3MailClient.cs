        ...
        public void Connect()
        {
            ...
            else
            {
                ...
                catch (Exception ex)
                {
                    ...
                    //General.Util.UI.MyMessageDialogSmall.Show("The server is not ready. Pleas try again to login.");
                }
                ...
                if (useSSL)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        //General.Util.UI.MyMessageDialogSmall.Show("Server " + popServer + " found, but cannot get SSL data stream.\nRuntime Error: " + ex.ToString());
                    ...
                    catch (Exception ex)
                    {
                        ...
                        //General.Util.UI.MyMessageDialogSmall.Show("Server " + popServer + " found, but problem with SSL Authentication.\nRuntime Error: " + ex.ToString());
                ...
                else
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        //General.Util.UI.MyMessageDialogSmall.Show("Server " + popServer + " found, but cannot get data stream (without SSL).\nRuntime Error: " + ex.ToString());
                ...
                catch (Exception ex)
                {
                    if (useSSL)
                    {
                        ...
                        //General.Util.UI.MyMessageDialogSmall.Show("Server " + popServer + " found, but cannot read from SSL stream.\nRuntime Error: " + ex.ToString());

                    }
                    else
                    {
                        throw new Pop3Exception("Server " + popServer + " found, but cannot read from stream (without SSL).\nRuntime Error: " + ex.ToString());
                        //General.Util.UI.MyMessageDialogSmall.Show("Server " + popServer + " found, but cannot read from stream (without SSL).\nRuntime Error: " + ex.ToString());
                ...
                if (!readSingleLine(out response))
                {
                    ...
                    //General.Util.UI.MyMessageDialogSmall.Show("Server " + popServer + " not ready to start AUTHORIZATION.\nMessage: " + response);
                ...
                if (!executeCommand("USER " + username, out response))
                {
                    ...
                    //General.Util.UI.MyMessageDialogSmall.Show("Server " + popServer + " doesn't accept username '" + username + "'.\nMessage: " + response);
                ...
                if (!executeCommand("PASS " + password, out response))
                {
                    ...
                    //General.Util.UI.MyMessageDialogSmall.Show("Server " + popServer + " doesn't accept password '" + password + "' for user '" + username + "'.\nMessage: " + response);
        ...
        private bool executeCommand(string command, out string response)
        {
            ...
            if (response == null)
            {
                ...
                if (response == null)
                {
                    ...
                    //General.Util.UI.MyMessageDialogSmall.Show("Server " + popServer + " has not responded, timeout has occurred.");                  
                }
            }
