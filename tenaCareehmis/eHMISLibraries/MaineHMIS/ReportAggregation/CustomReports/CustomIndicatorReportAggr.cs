...
namespace eHMIS.HMIS.ReportAggregation.CustomReports
{
    ...
    public class CustomIndicatorReportAggr : ICustomReport
    {
        ...
        public CustomIndicatorReportAggr(List<string> locations, int startMonth, int endMonth, 
            int yearStart, int yearEnd, int quarterStart, int quarterEnd, int repKind, 
            int repPeriod, bool showNumDenom, bool showOnlyQuartelyDE, 
            bool showTarget, bool isCacheEnabled)
        {
        ...
        public DataTable CreateReport()
        {
            ...
            SqlCommand toExecute;
            queryTable = "select * from EthEhmis_IndicatorsNewDisplay  "
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                targetDivide = Convert.ToBoolean(row["targetDivide"].ToString());
        ...
        private Hashtable getLocationIdLabelIdValue(out Hashtable locationIdMonthLabelValue)
        {
            ...
            string dataEleClasses = " and (dataEleClass = 6 or dataEleClass = 7 or dataEleClass = 8 or dataEleClass = 3 or dataEleClass = 2) ";
            ...
            int numDaysMonthTotal = monthCnt * 30;
            int numDaysMonth = 30;
            ...
            out decimal numMonthValue, out decimal denomMonthValue, int monthNum)
        {
            ...
            else
            {
                ...
                indicatorMonthValue = decimal.Round(numMonthValue, 2); ;
            }
            ...
            if (denomData == true)
            {
                ...
                if (singleFacility == true)
                {
                    if ((_repPeriod == 0) && (!onlyOneMonth))
                    {
                        denomMonthValue = decimal.Round(denomMonthValue, 2);
                    }
                    else if ((_repPeriod == 1) && (!onlyOneQuarter))
                    {
                        denomMonthValue = decimal.Round(denomMonthValue, 2);
            ...
            if (numData == true)
            {
                ...
                if (singleFacility == true)
                {
                    if ((_repPeriod == 0) && (!onlyOneMonth))
                    {
                        numMonthValue = decimal.Round(numMonthValue, 2);
                        
                        indicatorMonthValue = decimal.Round(indicatorMonthValue, 2);
                    }
                    else if ((_repPeriod == 1) && (!onlyOneQuarter))
                    {
                        numMonthValue = decimal.Round(numMonthValue, 2);
