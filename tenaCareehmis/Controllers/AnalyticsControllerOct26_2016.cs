...
namespace eHMISWebApi.Controllers
{
    ...
    public class AnalyticsController : ApiController
    {
            ...
            out string serviceParameterSql)
        {
            ...
            if (parameterValues.dataElements != null)
            {
                ...
                if (parameterValues.PeriodRange == "PeriodRangeYearly")
                {
                    ...
                    totalCalcText = " select labelId, VerticalSumID from EthioHIMS_ServiceDataElementsNew " +
                    ...
                    string labelIdsLastMonthText = 
                                        " select labelId from EthioHIMS_ServiceDataElementsNew " +
                    ...
                    string labelIdsLastMonthTotalText = 
                                       " select labelId, VerticalSumID from EthioHIMS_ServiceDataElementsNew " +
                ...
                {
                    fullDescriptionsAll += "\ncast([" + elements.FullDescription + "] as bigint) as [" +
                    ...
                    if (yearlyRange)
                    {
                        ...
                        else if (totalLabelIdListLastMonth.Contains(elements.labelId))
                        {
                            ...
                            // First get the Id
                            int id = Convert.ToInt16(totalLabelIdHashLastMonth[elements.labelId]);
                ...
                serviceParameterSql = getServiceQuery(parameterValues, fullDescriptionsAll,
                    ...
                    lastMonthCalc, totalCalcLabelId, totalCalcLabelIdLastMonth, facilityWhereQuery);
        ...
        private string getTotalAggregateQuery(AnalyticsParameters parameterValues, string labelIdsSumAll)
        {
            ...
            {
                frontListingAggrOption = aggregationGroupBy + aggregationIdsRows;
            }
            else if (parameterValues.dataElementsPivot == "pivotinstitutions")
            {
                frontListingAggrOption = aggregationOptions;
            }               
            else if (parameterValues.dataElementsPivot == "row")
            {
                frontListingAggrOption = aggregationOptions + "sequenceNo, " + aggregationIdsPivot;
            }
            ...
            " Value from \n " +
            "       ( \n" +
                "      select EthioHIMS_VerticalSumNew.ID, " +
                //"      FiscalYear, FiscalMonth, \n" +
                periodListing +
                //"      ReportingRegionName, ZoneName, WoredaName, \n" + 
                aggregationGroupBy + aggregationIdsRows +
                "      sum(value) as Value  \n " +
                "      from EthioHIMS_ServiceDataElementsNew    \n " +
                "      inner join EthioHIMS_VerticalSumNew on \n " +
                "      EthioHIMS_ServiceDataElementsNew.LabelID = EthioHIMS_VerticalSumNew.LabelId  \n " +
                "      inner  join v_EthEhmis_HmisValue  \n " +
                "      on v_EthEhmis_HmisValue.LabelID = EthioHIMS_ServiceDataElementsNew.LabelID   \n " +
                "      inner  join EthEhmis_AllFacilityWithID on \n " +
                "      EthEhmis_AllFacilityWithID.HMISCode = v_EthEhmis_HmisValue.LocationID \n " +
                "      where EthioHIMS_VerticalSumNew.ID in  " + "\n" +
                "      ( " + labelIdsSumAll + " ) " + "\n" +
                "       and (dataClass = 6 or dataClass =7 )" + institutionWhere + facilityWhereQuery + "\n" +
                 // "       FiscalYear = 2008 and FiscalMonth = 12 \n " +
                 periodWhereSelection + "\n" +             
                "       group by EthioHIMS_VerticalSumNew.ID, \n " +
            ...
            "       ) as t   \n " +
            "       inner join EthioHIMS_ServiceDataElementsNew \n " +
            "       on EthioHIMS_ServiceDataElementsNew.VerticalSumID = t.ID \n " +
            "       order by SequenceNo, " +
                // "       ReportingRegionName, ZoneName, WoredaName \n" +
        ...
        private string getLastMonthServiceQueryTotal(AnalyticsParameters parameterValues, string labelIdsSumAll, string labelIdsSumLastMonth)
        {
            ...
            if (parameterValues.dataElementsPivot == "row")
            {
                ...
                frontListingIds = vaggregationIdsRows + " EthioHIMS_ServiceDataElementsNew.sequenceNo, \n" +
                                  " EthioHIMS_ServiceDataElementsNew.FullDescription, v.FiscalYear,\n";

                if (labelIdsSumAll != "")
                {
                    orderByListing =
                         "    order by EthioHIMS_ServiceDataElementsNew.FullDescription, v.FiscalYear, \n" +
                   //"    v.ReportingRegionName, " +
                   vaggregationOptions + vaggregationIdsRows +
                   "    EthioHIMS_ServiceDataElementsNew.sequenceNo  \n";
                }
                else
                {
                    orderByListing =
                        "    order by FullDescription, FiscalYear, \n" +
                  //"    v.ReportingRegionName, " +
                  aggregationGroupBy + aggregationIdsPivot +
                  "    sequenceNo  \n";
                ...
                groupByListing =
                               "    group by EthioHIMS_ServiceDataElementsNew.FullDescription, v.FiscalYear, " +
               //"    v.ReportingRegionName, \n" +
               vaggregationOptions + vaggregationIdsRows +
               "    EthioHIMS_ServiceDataElementsNew.sequenceNo \n";
            }
           ...
           string totalLastMonthQuery =
                ...
                "   (   " +
                "            select EthioHIMS_VerticalSumNew.ID, FiscalYear, [FiscalMonth], \n" +
               // "            ReportingRegionName, EthEhmis_AllFacilityWithID.RegionId, \n" +
                ...
                "            inner join EthEhmis_AllFacilityWithID on   \n" +
                "            EthEhmis_AllFacilityWithID.HMISCode = v_EthEhmis_HmisValue.LocationID  \n" +
                "            where EthioHIMS_VerticalSumNew.ID in    (" + labelIdsSumAll + ")\n" +               
                "            and (dataClass = 6 or dataClass = 7)   \n" +                
              //  "            (FiscalYear >= 2008 and FiscalYear <= 2008) " +
                             periodWhereSelection +             
                "            group by " + aggregationGroupBy + aggregationIdsRows + " FiscalYear, FiscalMonth, EthioHIMS_VerticalSumNew.ID \n"    +
                //"            EthioHIMS_VerticalSumNew.ID,  " +
                //"            ReportingRegionName, EthEhmis_AllFacilityWithID.RegionId, FiscalYear, FiscalMonth  " +
                "    ) s    \n" +
            "    inner join \n" +
            "   (   \n" +
            "   select    " + aggregationGroupBy + aggregationIdsPivot + "\n" +
           // "   ReportingRegionName, RegionId, " + 
            ...
            "   FiscalYear, max(FiscalMonth) as FiscalMonth from    \n" +
               ...
               //  "            (FiscalYear >= 2008 and FiscalYear <= 2008) " +
                ...
                //"            ReportingRegionName, EthEhmis_AllFacilityWithID.RegionId, FiscalYear, FiscalMonth  " +
                "   ) t \n" +
            " group by " + aggregationGroupBy + aggregationIdsPivot + "\n" +
            "   ID, FiscalYear \n" +
            //" ReportingRegionName, RegionId    " +
            " ) v   " +
            "   on v.ID = s.ID and  \n" +
            ...
            "      v.fiscalYear = s.FiscalYear    \n" +          
                    lastMonthInnerJoins + "\n" +
            "      inner join EthioHIMS_ServiceDataElementsNew  \n" +
            "      on EthioHIMS_ServiceDataElementsNew.VerticalSumID = v.ID \n" +
            "      group by EthioHIMS_ServiceDataElementsNew.FullDescription, v.FiscalYear,     \n" +
            //"      v.ReportingRegionName,  v.RegionId,  " +
                   vaggregationOptions + vaggregationIdsRows + "\n" +
            "      EthioHIMS_ServiceDataElementsNew.sequenceNo \n" + 
                      
            "      order by FiscalYear,   \n" +
            //"      v.ReportingRegionName,   RegionId,     " +
                   vaggregationOptions + vaggregationIdsRows + "\n" +
            "      EthioHIMS_ServiceDataElementsNew.sequenceNo  \n";           
        ...
        private string getServiceQuery(AnalyticsParameters parameterValues, string fullDescriptions,
            ...
            bool totalCalcLabelIdLastMonth, string facilityWhereQuery)            
        {          
            ...
            string totalQuery = getTotalAggregateQuery(parameterValues, labelIdsSumAllTotal);
